...
var ASTEROID_DEFAULT_TARGET_OFFSET = 2;
var ASTEROID_DEFAULT_SPAWN_OFFSET = 20;
var ASTEROID_DEFAULT_SPAWN_DELAY = 2;
var ASTEROID_MIN_SPAWN_DELAY = 0.5;
var ASTEROID_SPAWN_DELAY_SUB = 0.02;
var ASTEROID_COUNT = 11;
var ASTEROID_MAX_SPEED = 50;
...
exports.init = function() {
    // registrate asynchronous devices
    ...
    m_app.init({
        ...
        // change scene graph
        stereo: m_input.can_use_device(m_input.DEVICE_HMD) ? "HMD": "NONE"
    });
}
...
function register_gamepad(is_hmd) {
    ...
    var controller_cb = function(obj, id, pulse) {
        ...
        var axes = m_ctl.get_sensor_payload(obj, id, 8);
        var elapsed = m_ctl.get_sensor_value(obj, id, 9);
        var time = m_ctl.get_sensor_value(obj, id, 10);
        ...
        _dest_x_trans += velocity.trans * COCKPIT_TRANS_FACTOR * elapsed * axes[0];
        _dest_y_trans -= velocity.trans * COCKPIT_TRANS_FACTOR * elapsed * axes[1];

        if (!is_hmd) {
            var cam_angles = m_cam.get_camera_angles(obj, _vec2_tmp);
            var vert_ang = cam_angles[1] - axes[3] * rot_value;
            var hor_ang = - axes[2] * rot_value;
    ...
    var init_sensors = function(index) {
        var gs_w = m_ctl.create_gamepad_btns_sensor(index, m_input.DPAD_DOWN_GAMEPAD_BTN);
        var gs_d = m_ctl.create_gamepad_btns_sensor(index, m_input.DPAD_LEFT_GAMEPAD_BTN);
        var gs_s = m_ctl.create_gamepad_btns_sensor(index, m_input.DPAD_UP_GAMEPAD_BTN);
        var gs_a = m_ctl.create_gamepad_btns_sensor(index, m_input.DPAD_RIGHT_GAMEPAD_BTN);

        var gs_r1 = m_ctl.create_gamepad_btns_sensor(index, m_input.R1_GAMEPAD_BTN);
        var gs_r2 = m_ctl.create_gamepad_btns_sensor(index, m_input.R2_GAMEPAD_BTN);
        var gs_l1 = m_ctl.create_gamepad_btns_sensor(index, m_input.L1_GAMEPAD_BTN);
        var gs_l2 = m_ctl.create_gamepad_btns_sensor(index, m_input.L2_GAMEPAD_BTN);

        var axes_sensor = m_ctl.create_gamepad_axes_sensor(0);
        ...
        m_ctl.create_sensor_manifold(cam_obj, "CONTROLLER_CAMERA_MOVE" + index,
                ...
                gs_l1, gs_l2, axes_sensor, e_s, time],
...
function register_mouse(is_hmd) {
    ...
    m_ctl.create_sensor_manifold(null, "MOUSE_SHOOT",
            m_ctl.CT_POSITIVE, [time, mclick], null, shoot_cb);
}
...
function register_cockpit() {
    ...
    if (cockpit_mesh_obj && Boolean(user_media)) {
        ...
        var fail_cb = function() {
            console.error("Couldn't initialize getUserMedia.")
        };
