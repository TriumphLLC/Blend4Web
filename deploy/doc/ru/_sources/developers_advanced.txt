...
Для перевода новых текстовых строк необходимо выполнить ряд подготовительных действий.

Первым делом убедитесь, что у вас установлена программа *Subversion*. Затем склонируйте репозиторий с переводами Blender'a. Сделать это можно следующей командой:

.. code-block:: bash

    svn checkout https://svn.blender.org/svnroot/bf-translations 

Вторым шагом будет подключение и настройка аддона *Manage UI translations*. Самой важной настройкой тут является "Translation Root", где необходимо указать путь к склонированному репозиторию. Затем выберите абсолютный путь для сохранения настроек в поле "Persistent Data Path" и сохраните настройки в .json-файл.

.. image:: src_images/developers_advances/translator.png
   :align: center
   :width: 100%

Затем необходимо вызвать скрипт ``translator.py`` из директории ``scripts/`` для обновления .po-файла:

.. code-block:: bash

    python3 scripts/translator.py ru_RU /home/user/bf-translations/branches/ru/ru.po

Первым параметром необходимо указать язык, на который будет переводиться аддон. Список доступных языков можно посмотреть следующим образом:

.. code-block:: bash

    python3 scripts/translator.py help

Второй параметр является опциональным и служит для слияния полученного .po-файла с .po-файлом Blender'a, который доступен из описаного выше репозитория в директории */bf-translations/branches/*. Совпадающие фразы из вашего .po-файла с .po-файлом Blender'a переведутся автоматически.

Теперь осталось открыть полученный файл и написать перевод новых строк.
