...
Вторым шагом будет подключение и настройка аддона *Manage UI translations*. Единственно важной настройкой является ``Translation root``, где необходимо указать путь к репозиторию из первого шага.

.. image:: src_images/developers_advances/translator.png
   :align: center
   :width: 100%

Затем необходимо вызвать скрипт translator.py из директории /scripts для обновления файла *ru_RU.po*:

.. code-block:: bash

    python3 scripts/translator.py

Теперь осталось открыть этот файл и написать перевод новых строк.