...
2. Разместить перед камерой поверхность с материалом и опциональной текстурой, присоединить ее к камере связью родитель-потомок. При необходимости настроить расстояние до поверхности, переднюю и заднюю плоскости отсечения камеры.

.. image:: src_images/lighting/parented_background.jpg
   :align: center
   :width: 100%

3. Использовать :ref:`текстуру неба <skydome_texture>`.

4. Настроить динамически генерируемую :ref:`атмосферу <atmosphere>`.

5. Установить параметр движка ``background_color``, используя программный метод ``config.set()``. Установленное значение используется в качестве аргумента метода WebGL ``clearColor()``. Для получения корректных результатов рекомендуется отключить прозрачность контекста WebGL (параметр ``alpha``). Такая конфигурация используется по умолчанию в стандартном веб-плеере движка.
