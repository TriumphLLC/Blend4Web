...
.. image:: src_images/addon/addon_dev_tools.png
   :align: center
   :width: 100%
...
This button is duplicated in the ``Info`` panel of the :ref:`3D View window <ui_3dview>`.

.. figure:: src_images/addon/addon_fast_preview.png
   :align: center
   :width: 100%

   ``Fast Preview`` button on the ``Info`` panel

The button at the right side of it controls data syncronization between Blender and :ref:`Scene Viewer <viewer>`. If this option is enabled, re-exporting a scene will cause Scene Viewer application (if it is open) to reload and update the scene in accordance to the changes made to it. This option eliminates the need to open preview scene manually after exporting it. It is enabled by default.

.. note::

    The ``Fast Preview`` button in the ``Info`` panel is available in ``Cycles`` render engine as well.
