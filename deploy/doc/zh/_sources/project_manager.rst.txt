...
10) Web Player URL attributes. This tab is available only if ``Engine Binding Type`` parameter is set to ``Web Player JSON`` or ``Web Player HTML``.
    ...
    * "Use compressed textures" enables loading of minified and :ref:`compressed textures <converter_data_format>` (in DDS format).
    * "Use compressed textures pvr" enables loading of minified and :ref:`compressed textures <converter_data_format>` (in PVR format).
    * "Use min capabilities" makes Web Player run application as if it was running on a low-end system (on a mobile device, for example).
    * "Use gzip compression" enables using :ref:`gzip <gzip>` compression.
