...
J.create_kb_sensor_manifold(a,"QUIT",J.CT_SHOT,J.KEY_Q,function(a,b,d,e){na=null;c();I.disable_camera_controls(a);J.remove_sensor_manifold(a,"ZOOM_TO")});var b=J.create_keyboard_sensor(J.KEY_Z),d=J.create_keyboard_sensor(J.KEY_DEC_POINT);J.create_sensor_manifold(a,"ZOOM_TO",J.CT_SHOT,[b,d],function(a){return a[0]||a[1]},function(a,b,c){(b=T)&&Wa.zoom_object(a,b)})}function c(){if(Aa){var a=na?x.get_object_name(na):"NONE",b=T?x.get_object_name(T):"NONE",a="CONTROLLED: "+a+" | SELECTED: "+b;(b=T)&&
(b=x.get_type_mesh_object(b))&&(a+=" | TYPE: "+b);b=na;T&&b&&"CAMERA"===b.type&&(a+=" | DISTANCE: "+Xa);Aa.innerHTML=a}}function d(){var a=H.get("last_item_name");if(!a)return null;for(var b=0;b<X.length;b++){var c=X[b];if(va.keyfind("name",a,c.items)[0])return{category:c.name,item:a}}return null}function b(a,b){var c=document.getElementById(a);c.innerHTML="";for(var d=0;d<b.length;d++){var e=document.createElement("option");e.text=e.value=b[d];0===d&&(e.selected=!0);c.appendChild(e)}$("#"+a).selectmenu();
$("#"+a).selectmenu("refresh")}function f(){var a=document.getElementById("default_scene");a&&(a.onclick=function(){wa=!1;clearTimeout(Ka);h(null,!0,!1,!0)})}function e(a){a=a.target.id;if(-1!=a.indexOf("MANIFEST_ITEM_")){var b=a.split("__SEPARATOR__");a=b[1];b=b[0].split("MANIFEST_ITEM_")[1];wa=!1;clearTimeout(Ka);h({category:a,item:b},!0,!1,!1)}}function h(b,c,d,e){if(N)for(var f=0;f<N.children.length;f++)N.children[f].style.visibility="hidden";if((f=I.get_url_params())&&f.load){var g=f.load.split("/");
sa={load_file:f.load,name:g[g.length-1].split(".")[0],animated_objects:[]}}else sa={load_file:K.get_std_assets_path()+"misc/logo.json",name:"Logo",animated_objects:[]};ca=E=null;if(!e)if(b){e=va.keyfind("name",b.category,X)[0];e=va.keyfind("name",b.item,e.items)[0];for(var h in e)sa[h]="load_file"==h?K.get_std_assets_path()+e[h]:e[h];H.set("last_item_name",b.item)}else H.set("last_item_name","");c&&(ma.clear_render_callback(),La.cleanup());ma.get_canvas_elem().removeEventListener("mousedown",a);Q.clear_errors_warnings();
b=sa.name;c=sa.load_file;h=document.getElementById("current_file");h.innerHTML=b;h.setAttribute("title",b+" ("+c+")");La.load(c,l,p,d)}function l(b){ma.get_canvas_elem().addEventListener("mousedown",a,!1);T=null;q(sa);g();I.enable_debug_controls();B()&&kb.enable_mixer_controls();K.get("gyro_use")&&lb.enable_camera_rotation();ma.set_render_callback(w);wa&&(Ka=setTimeout(m,1E3));b=J.create_elapsed_sensor();J.create_sensor_manifold(x.get_active_camera(),"TO_OBJ",J.CT_CONTINUOUS,[b],null,function(a,b,
d){if(1==d&&T){if(d=T)b=mb,nb.get_object_center(d,!0,b),a=Wa.get_eye(a,ob),Xa=pb.dist(b,a).toFixed(3);c()}})}function m(){var a=d();if(a){var b=a.item,a=va.keyfind("name",a.category,X)[0],b=va.keyfind("name",b,a.items)[0],b=a.items.indexOf(b);if(b=a.items[b+1])a={category:a.name,item:b.name};else{a=X.indexOf(a);b=null;do b=X[++a];while(b&&0==b.items.length);if(b)a={category:b.name,item:b.items[0].name};else{wa=!1;return}}}else a=X[0],b=a.items[0],a={category:a.name,item:b.name};h(a,!0,!0,!1)}function p(a,
b){document.getElementById("loading_progress").innerHTML=a+"% ("+Math.round(10*b/1E3)/10+"s)";var c=Q.get_error_quantity(),d=Q.get_warning_quantity();if(La.is_primary_loaded()){var e=Q.num_vertices(),f=Q.num_triangles(),g=Q.num_draw_calls(),h=Q.num_shaders();document.getElementById("info_right_up").innerHTML=e+" verts, "+f+" tris, "+g+" draw calls, "+h+" shaders";var e=Q.geometry_stats(),f=Q.num_textures(),g=Q.num_render_targets(),h=Math.round(10*(e.ibo_memory+e.vbo_memory))/10,k=Math.round(10*f.memory)/
10,l=Math.round(10*g.memory)/10,m=Math.round(10*(h+k+l))/10;document.getElementById("info_right_down").innerHTML="Geometry: "+h+" MiB ("+e.ibo_number+"+"+e.vbo_number+")<br>Textures: "+k+" MiB ("+f.number+")<br>RTs: "+l+" MiB ("+g.number+")<br><br>Total: "+m+" MiB";e=Q.get_error_quantity();(f=Q.get_warning_quantity())||e?N.setAttribute("title","Errors: "+e+", Warnings: "+f+"\nSee browser console for more info"):N.setAttribute("title","Loaded OK");d||c||(N.children[0].style.visibility="hidden",N.children[1].style.visibility=
"hidden",N.children[2].style.visibility="visible")}d&&!c&&(N.children[0].style.visibility="hidden",N.children[1].style.visibility="visible",N.children[2].style.visibility="hidden");c&&(N.children[0].style.visibility="visible",N.children[1].style.visibility="hidden",N.children[2].style.visibility="hidden")}function q(a){for(var c=x.get_active(),d=x.get_scenes(),e=0;e<d.length;e++){var f=d[e];if(f!==c){var g=ha(f,a);D(f,g)}}g=ha(c,a);D(c,g);Ua=function(a){var c=qa.get_materials_names(a);b("material_name",
c);Ya(a,c[0]);a=qa.get_water_material_params(a,c[0]);c="shallow_water_col shallow_water_col_fac shore_water_col shore_water_col_fac water_fog_color water_fog_density absorb_factor sss_strength sss_width foam_factor norm_uv_velocity waves_height waves_length dst_noise_scale0 dst_noise_scale1 dst_noise_freq0 dst_noise_freq1 dir_min_shore_fac dir_freq dir_noise_scale dir_noise_freq dir_min_noise_fac dst_min_fac waves_hor_fac water_dynamic shore_smoothing".split(" ");a?(z(c,"enable"),x.get_water_mat_params(a),
a.shallow_water_col&&ga("shallow_water_col",a.shallow_water_col),a.shallow_water_col_fac&&u("shallow_water_col_fac",a.shallow_water_col_fac),a.shore_water_col&&ga("shore_water_col",a.shore_water_col),a.shore_water_col_fac&&u("shore_water_col_fac",a.shore_water_col_fac),a.water_fog_color&&(ga("water_fog_color",a.water_fog_color),u("water_fog_density",a.water_fog_density)),u("absorb_factor",a.absorb_factor),u("sss_strength",a.sss_strength),u("sss_width",a.sss_width),u("foam_factor",a.foam_factor),u("norm_uv_velocity",
a.norm_uv_velocity),u("waves_height",a.waves_height),u("waves_length",a.waves_length),u("dst_noise_scale0",a.dst_noise_scale0),u("dst_noise_scale1",a.dst_noise_scale1),u("dst_noise_freq0",a.dst_noise_freq0),u("dst_noise_freq1",a.dst_noise_freq1),u("dir_min_shore_fac",a.dir_min_shore_fac),u("dir_freq",a.dir_freq),u("dir_noise_scale",a.dir_noise_scale),u("dir_noise_freq",a.dir_noise_freq),u("dir_min_noise_fac",a.dir_min_noise_fac),u("dst_min_fac",a.dst_min_fac),u("waves_hor_fac",a.waves_hor_fac)):z(c,
"disable")};qb();a=x.get_all_objects("LAMP");c=[];for(e=0;e<a.length;e++)c.push(a[e].name);b("light_name",c);0<a.length&&Za(a[0]);rb();cb();sb();tb();ub();db();vb();wb();xb();yb();zb();Ab();Bb();Va();$("#wireframe_mode").val("WM_NONE");$("#wireframe_mode").selectmenu("refresh",!0);$("#wireframe_edge_color div").css("backgroundColor","#333");e=new Date(document.getElementById("date").value);Ga({date:e})}function ha(a,b){function c(a,b){for(var d in a)b[d]=a[d];return b}var d=b.scene_settings;if(d&&
d[a]){var e={};c(b,e);c(d[a],e);delete e.scene_settings;return e}return b}function D(a,c){x.set_active(a);x.get_active_camera();for(var d=[],e=x.get_all_objects(),f=0;f<e.length;f++){var g=e[f];F.is_animated(g)&&(E||(E=g),d.push(x.get_object_name(g)))}f="anim_active_object anim_slot animation anim_cyclic anim_frame_range anim_mix_factor anim_frame_current anim_play anim_stop".split(" ");E?(z(f,"enable"),b("anim_active_object",d),ea({anim_active_object:x.get_object_name(E)})):z(f,"disable");d=["nla_play",
"nla_stop","nla_status","nla_frame_current","nla_playing"];f=["nla_frame_range"];if(U.check_nla()){U.check_nla_scripts()?(z(d,"disable"),document.getElementById("nla_status").innerHTML="CONTROLLED BY NLA SCRIPT"):z(d,"enable");z(f,"enable");var d=document.getElementById("nla_frame_current"),f=document.getElementById("nla_frame_range"),g=U.get_frame_start(),h=U.get_frame_end();-1!=g&&-1!=h?(f.innerHTML=g+" - "+h,d.min=""+g,d.max=""+h):f.innerHTML="N/A"}else z(d,"disable"),z(f,"disable");d=[];for(f=
0;f<e.length;f++)g=e[f],xa.check_shape_keys(g)&&(ca||(ca=g),d.push(x.get_object_name(g)));e=["shape_key_obj","shape_key_name","shape_key_value"];ca?(z(e,"enable"),b("shape_key_obj",d),S({shape_key_obj:x.get_object_name(ca)})):z(e,"disable")}function w(a,b){x.get_active_camera();if(E){var c=parseInt(document.getElementById("anim_slot").value),d=Math.round(F.get_frame(E,c));parseInt($("#anim_frame_current").val())!==d&&u("anim_frame_current",d);d=document.getElementById("anim_status");F.is_play(E,c)?
d.innerHTML="PLAYING":d.innerHTML="STOPPED"}U.check_nla()&&(U.check_nla_scripts()||(d=document.getElementById("nla_status"),U.is_play()?d.innerHTML="PLAYING":d.innerHTML="STOPPED"),d=parseInt(U.get_frame().toFixed(0)),parseInt($("#nla_frame_current").val())!==d&&u("nla_frame_current",d))}function y(a){H.set("quality",a.quality);setTimeout(function(){window.location.reload()},100)}function A(){H.cleanup();window.location.reload()}function C(){ma.pause()}function v(){ma.resume()}function B(){return"true"===
H.get("sfx_mix_mode")?!0:!1}function R(a){H.set("show_hud_debug_info",a.show_hud_debug_info);window.location.reload()}function O(a){H.set("sfx_mix_mode",a.sfx_mix_mode);window.location.reload()}function L(a){H.set("anaglyph_use",a.anaglyph_use);window.location.reload()}function ja(a){H.set("gyro_use",a.gyro_use);window.location.reload()}function ka(a){H.set("outlining_overview_mode",a.outlining_overview_mode);window.location.reload()}function W(){wa=!0;m()}function la(a){I.resize_to_container();a=
ma.get_canvas_elem();document.getElementById("info_left_up").innerHTML=a.width+"x"+a.height+" ("+a.width/a.clientWidth+":1)"}function Y(){var a=document.getElementById("animation").value,b=parseInt(document.getElementById("anim_slot").value);"None"!==a&&F.play(E,null,b)}function ta(){document.getElementById("animation");var a=parseInt(document.getElementById("anim_slot").value);F.stop(E,a)}function M(){U.play()}function Z(){U.stop()}function S(a){if("shape_key_obj"in a){ca=x.get_object_by_name(a.shape_key_obj);
var c=xa.get_shape_keys_names(ca);c.length?(b("shape_key_name",c),ya=c[0],c=xa.get_shape_key_value(ca,ya),u("shape_key_value",c)):b("shape_key_name",["N/K"])}"shape_key_name"in a&&(ya=a.shape_key_name,c=xa.get_shape_key_value(ca,ya),u("shape_key_value",c));"shape_key_value"in a&&xa.set_shape_key_value(ca,ya,a.shape_key_value)}function ia(a){"nla_frame_current"in a&&!U.is_play()&&(a=parseInt(a.nla_frame_current),U.set_frame(a))}function ea(a){if("anim_active_object"in a){E=x.get_object_by_name(a.anim_active_object);
var c=F.get_anim_names(E);if(c.length){for(var d=[],e=0;e<c.length&&8>e;e++)d.push(e);c.push("None");b("animation",c);b("anim_slot",d);c=parseInt(document.getElementById("anim_slot").value);Na(E,c);Ca(E,c)}else b("animation",["N/A"]);Da(E)}if("anim_slot"in a){if(!E)return;c=a.anim_slot;Na(E,c);Ca(E,c);Da(E)}"animation"in a&&(c=parseInt(document.getElementById("anim_slot").value),F.get_current_anim_name(E,c),"None"==a.animation?F.remove_slot_animation(E,c):(F.apply(E,a.animation,c),F.play(E,null,c)),
Ca(E,c),Da(E));"anim_cyclic"in a&&(c=document.getElementById("animation").value,c=F.get_slot_num_by_anim(E,c),F.set_behavior(E,a.anim_cyclic?F.AB_CYCLIC:F.AB_FINISH_RESET,c));"anim_frame_current"in a&&(c=parseInt(document.getElementById("anim_slot").value),F.is_play(E,c)||(d=parseInt(a.anim_frame_current),F.set_frame(E,d,c)));"anim_mix_factor"in a&&F.set_skel_mix_factor(E,a.anim_mix_factor)}function Na(a,b){for(var c=F.get_current_anim_name(a,b),c=c||"None",d=$("#animation"),e=d.children(),f=0;f<
e.length;f++){var g=e[f];if(g.value==c){g.selected=!0;d.selectmenu();d.selectmenu("refresh");break}}}function Ca(a,b){u("anim_cyclic",Number(F.get_behavior(a,b)==F.AB_CYCLIC));var c=document.getElementById("anim_frame_range"),d=F.get_anim_start_frame(a,b),e=F.get_anim_length(a,b);-1!=d&&-1!=e?(e=d+e,c.innerHTML=d+" - "+e,c=document.getElementById("anim_frame_current"),c.min=""+d,c.max=""+e):c.innerHTML="N/A"}function Da(a){for(var b=$("#anim_slot-listbox").find("a"),c=parseInt(document.getElementById("anim_slot").value),
...
"disable")}function fa(a){x.get_ssao_params();var b={};"ssao_quality"in a&&(b.ssao_quality=$a(document.getElementById("ssao_quality")));"ssao_hemisphere"in a&&(b.ssao_hemisphere=Number(a.ssao_hemisphere));"ssao_blur_depth"in a&&(b.ssao_blur_depth=Number(a.ssao_blur_depth));"ssao_blur_discard_value"in a&&(b.ssao_blur_discard_value=a.ssao_blur_discard_value);"ssao_radius_increase"in a&&(b.ssao_radius_increase=a.ssao_radius_increase);"ssao_influence"in a&&(b.ssao_influence=a.ssao_influence);"ssao_dist_factor"in
a&&(b.ssao_dist_factor=a.ssao_dist_factor);"ssao_only"in a&&(b.ssao_only=Number(a.ssao_only));"ssao_white"in a&&(b.ssao_white=Number(a.ssao_white));x.set_ssao_params(b)}function db(){var a=x.get_color_correction_params(),b=["brightness","contrast","exposure","saturation"];a?(z(b,"enable"),u("brightness",a.brightness),u("contrast",a.contrast),u("exposure",a.exposure),u("saturation",a.saturation)):z(b,"disable")}function za(a){var b=x.get_color_correction_params();b&&(b={},"brightness"in a&&(b.brightness=
parseFloat(a.brightness)),"contrast"in a&&(b.contrast=parseFloat(a.contrast)),"exposure"in a&&(b.exposure=parseFloat(a.exposure)),"saturation"in a&&(b.saturation=parseFloat(a.saturation)),x.set_color_correction_params(b))}function $a(a){a=a.options;for(var b=0;b<a.length;b++){var c=a[b];if(c.selected)return c.value}}function sb(){var a=x.get_fog_color_density();.2<a[3]?u("fog_density",a[3]):u("fog_density1000",1E3*a[3]);ga("fog_color",a)}function Ha(a){var b=x.get_fog_color_density();"fog_density"in
a&&(b[3]=a.fog_density);"fog_density1000"in a&&(b[3]=a.fog_density1000/1E3);"fog_color"in a&&(b[0]=a.fog_color[0],b[1]=a.fog_color[1],b[2]=a.fog_color[2]);x.set_fog_color_density(b)}function rb(){var a=x.get_shadow_params(),b="csm_first_cascade_border csm_last_cascade_border self_shadow_polygon_offset self_shadow_normal_offset first_cascade_blur_radius last_cascade_blur_radius res_ui-block-b num_ui-block-b blur_ui-block-b".split(" ");if(!a)return z(b,"disable"),document.getElementById("csm_resolution").innerHTML=
"",document.getElementById("csm_num").innerHTML="",document.getElementById("csm_borders").innerHTML="",document.getElementById("blur_radii").innerHTML="",null;z(b,"enable");ab(a);u("self_shadow_polygon_offset",a.self_shadow_polygon_offset);u("self_shadow_normal_offset",a.self_shadow_normal_offset);u("first_cascade_blur_radius",a.first_cascade_blur_radius);a.enable_csm?(u("csm_first_cascade_border",a.csm_first_cascade_border),1<a.csm_num?(u("csm_last_cascade_border",a.csm_last_cascade_border),u("last_cascade_blur_radius",
a.last_cascade_blur_radius)):z(["csm_last_cascade_border","last_cascade_blur_radius"],"disable")):z(["csm_first_cascade_border","csm_last_cascade_border","last_cascade_blur_radius"],"disable")}function ra(a){var b={};"csm_first_cascade_border"in a&&(b.csm_first_cascade_border=a.csm_first_cascade_border);"csm_last_cascade_border"in a&&(b.csm_last_cascade_border=a.csm_last_cascade_border);"self_shadow_polygon_offset"in a&&(b.self_shadow_polygon_offset=a.self_shadow_polygon_offset);"self_shadow_normal_offset"in
...
document.getElementById("first_cascade_blur_radius-label").innerHTML="Blur radius",document.getElementById("last_cascade_blur_radius-info-block").style.display="none"}function qb(){var a=x.get_environment_colors();u("environment_energy",a[0]);ga("horizon_color",a[1]);ga("zenith_color",a[2])}function Fa(a){"environment_energy"in a&&x.set_environment_colors(a.environment_energy,null,null);"horizon_color"in a&&x.set_environment_colors(null,a.horizon_color,null);"zenith_color"in a&&x.set_environment_colors(null,
null,a.zenith_color)}function Ta(a){var b={};"string"==typeof a.wireframe_mode&&(b.wireframe_mode=$a(document.getElementById("wireframe_mode")));"object"==typeof a.wireframe_edge_color&&(b.wireframe_edge_color=a.wireframe_edge_color);Q.set_debug_params(b)}function jb(){Cb.shot()}function xb(){var a=x.get_sky_params(),b="sky_color rayleigh_brightness mie_brightness spot_brightness scatter_strength rayleigh_strength mie_strength rayleigh_collection_power mie_collection_power mie_distribution".split(" ");
if(!a)return z(b,"disable"),null;z(b,"enable");ga("sky_color",a.color);u("rayleigh_brightness",a.rayleigh_brightness);u("mie_brightness",a.mie_brightness);u("spot_brightness",a.spot_brightness);u("scatter_strength",a.scatter_strength);u("rayleigh_strength",a.rayleigh_strength);u("mie_strength",a.mie_strength);u("rayleigh_collection_power",a.rayleigh_collection_power);u("mie_collection_power",a.mie_collection_power);u("mie_distribution",a.mie_distribution)}function ba(a){var b={};"sky_color"in a&&
...
a&&(b.mie_collection_power=a.mie_collection_power);"mie_distribution"in a&&(b.mie_distribution=a.mie_distribution);x.set_sky_params(b)}function Ya(a,b){var c=qa.get_material_extended_params(a,b),d="material_diffuse_color material_name material_reflectivity material_fresnel material_fresnel_factor material_parallax_scale material_parallax_steps".split(" ");if(!c)return z(d,"disable"),$("#material_warning")[0].style.display="block",null;var e=qa.get_diffuse_color(a,b);c.diffuse_color=e.subarray(0,3);
z(d,"enable");ga("material_diffuse_color",c.diffuse_color);u("material_reflectivity",c.reflect_factor);u("material_fresnel",c.fresnel);u("material_fresnel_factor",c.fresnel_factor);u("material_parallax_scale",c.parallax_scale);u("material_parallax_steps",c.parallax_steps)}function oa(a){var b=T;"material_name"in a&&Ya(b,a.material_name);if("material_diffuse_color"in a){var c=a.material_diffuse_color.slice();c.push(1);qa.set_diffuse_color(b,$("#material_name").val(),c)}c={};"material_reflectivity"in
a&&(c.material_reflectivity=a.material_reflectivity);"material_fresnel"in a&&(c.material_fresnel=a.material_fresnel);"material_fresnel_factor"in a&&(c.material_fresnel_factor=a.material_fresnel_factor);"material_parallax_scale"in a&&(c.material_parallax_scale=a.material_parallax_scale);"material_parallax_steps"in a&&(c.material_parallax_steps=a.material_parallax_steps);qa.set_material_extended_params(b,$("#material_name").val(),c);x.update_scene_materials_params()}function G(a){var b=T,c="waves_height waves_length sss_strength sss_width dst_noise_scale0 dst_noise_scale1 dst_noise_freq0 dst_noise_freq1 dir_min_shore_fac dir_freq dir_noise_scale dir_noise_freq dir_min_noise_fac dst_min_fac waves_hor_fac".split(" ");
if(!b)return null;var d={},e=$("#water_dynamic").val(),f=$("#shore_smoothing").val();Boolean(Number(e))?z(c,"enable"):z(c,"disable");Boolean(Number(f))?z(["absorb_factor"],"enable"):z(["absorb_factor"],"disable");"shore_smoothing"in a&&(d.shore_smoothing=a.shore_smoothing);"absorb_factor"in a&&(d.absorb_factor=a.absorb_factor);"water_fog_color"in a&&(d.water_fog_color=a.water_fog_color);"water_fog_density"in a&&(d.water_fog_density=a.water_fog_density);"shallow_water_col"in a&&(d.shallow_water_col=
...
u("pre_light_energy",b);u("rude_light_energy",c);b=c+b;$("#light_energy").text(b);b=a.light_type;$("#light_type").text(b);"SPOT"==a.light_type?(z(["light_spot_blend","light_spot_size"],"enable"),"light_spot_blend"in a&&u("light_spot_blend",a.light_spot_blend),"light_spot_size"in a&&u("light_spot_size",Math.round(a.light_spot_size*Db))):z(["light_spot_blend","light_spot_size"],"disable");"SPOT"==a.light_type||"POINT"==a.light_type?(z(["light_distance"],"enable"),"light_distance"in a&&u("light_distance",
a.light_distance)):z(["light_distance"],"disable")}}function pa(a){"light_name"in a&&Za(x.get_object_by_name(a.light_name));var b={};"light_color"in a&&(b.light_color=a.light_color);"pre_light_energy"in a&&(b.light_energy=parseFloat(a.pre_light_energy)+parseFloat(document.getElementById("rude_light_energy").value));"rude_light_energy"in a&&(b.light_energy=parseFloat(a.rude_light_energy)+parseFloat(document.getElementById("pre_light_energy").value));"light_spot_size"in a&&(b.light_spot_size=parseFloat(a.light_spot_size*
