...
r(va,"dof_power","number");r(va,"dof_on","bool");r(La,"god_rays_intensity","number");r(La,"god_rays_max_ray_length","number");r(La,"god_rays_steps","number");r(Da,"brightness","number");r(Da,"contrast","number");r(Da,"exposure","number");r(Da,"saturation","number");r(ha,"sfx_mix_mode","bool");a=Number("true"===F.get("sfx_mix_mode"));document.getElementById("sfx_mix_mode").options[a].selected=!0;$("#sfx_mix_mode").slider("refresh");r(ia,"stereo","string");a=F.get("stereo")||"NONE";Xa.can_use_device(Xa.DEVICE_HMD)||
(document.getElementById("stereo").options[2].setAttribute("disabled","disabled"),$("#stereo").selectmenu("refresh"),"HMD"==a&&(F.set("stereo","NONE"),a="NONE"));$("#stereo").val(a).selectmenu("refresh");$("#stereo").selectmenu("refresh",!0);I.set_onclick("hmd_settings",X);r(Ga,"gyro_use","bool");a=Number("true"===F.get("gyro_use"));document.getElementById("gyro_use").options[a].selected=!0;$("#gyro_use").slider("refresh");r(Z,"gmpd_settings","bool");r(Ya,"wind_dir","number");r(Ya,"wind_strength",
"number");r(Za,"mb_factor","number");r(Za,"mb_decay_threshold","number");r(Ma,"bloom_key","number");r(Ma,"bloom_blur","number");r(Ma,"bloom_edge_lum","number");r(Ea,"small_glow_mask_coeff","number");r(Ea,"large_glow_mask_coeff","number");r(Ea,"small_glow_mask_width","number");r(Ea,"large_glow_mask_width","number");r(tb,"canvas_rf","number");r($a,"debug_view_mode","string");I.set_onclick("debug_change_colors",ub);r(vb,"render_time_threshold","number");da($a,"wireframe_edge_color","object");r(Q,"show_hud_debug_info",
"bool");r(L,"enable_gl_debug","bool");r(ma,"outlining_overview_mode","bool");a=""==F.get("outlining_overview_mode")?1:Number("true"===F.get("outlining_overview_mode"));document.getElementById("outlining_overview_mode").options[a].selected=!0;$("#outlining_overview_mode").slider("refresh");I.set_onclick("make_screenshot",wb);a=Number("true"===F.get("show_hud_debug_info"));document.getElementById("show_hud_debug_info").options[a].selected=!0;$("#show_hud_debug_info").slider("refresh");a=""==F.get("enable_gl_debug")?
...
188==a.keyCode&&u(b,c-d);190==a.keyCode&&u(b,c+d)}})})}function a(a){if(Ca&&A.can_select_objects()){$("#material_warning")[0].style.display="none";a.preventDefault&&a.preventDefault();var b=a.clientX;a=a.clientY;var d=U;d&&A.outlining_is_enabled(d)&&A.clear_outline_anim(d);b=A.pick_object(b,a);b&&xb.is_empty(b)||((U=b)?ab(b):bb(),c(),ja||(b?(ja=b,c(),I.enable_object_controls(b),K.create_kb_sensor_manifold(b,"QUIT",K.CT_SHOT,K.KEY_Q,function(a,b,d,e){ja=null;c();I.disable_object_controls(a)})):g()))}}
function g(){var a=A.get_active_camera();ja=a;c();I.enable_camera_controls(!1,!1,!1,wa.get_container(),!0);K.create_kb_sensor_manifold(a,"QUIT",K.CT_SHOT,K.KEY_Q,function(a,b,d,e){ja=null;c();I.disable_camera_controls(a);K.remove_sensor_manifold(a,"ZOOM_TO")});var b=K.create_keyboard_sensor(K.KEY_Z),d=K.create_keyboard_sensor(K.KEY_DEC_POINT);K.create_sensor_manifold(a,"ZOOM_TO",K.CT_SHOT,[b,d],function(a){return a[0]||a[1]},function(a,b,c){(b=U)&&cb.target_zoom_object(a,b)})}function c(){if(Ca){var a=
ja?ka(ja):"NONE",b=U?ka(U):"NONE",a="CONTROLLED: "+a+" | SELECTED: "+b;(b=U)&&(b=A.get_type_mesh_object(b))&&(a+=" | TYPE: "+b);b=ja;U&&b&&"CAMERA"===b.type&&(a+=" | DISTANCE: "+db);Ca.innerHTML=a}}function d(){var a=F.get("last_item_name");if(!a)return null;for(var b=0;b<Y.length;b++){var c=Y[b];if(xa.keyfind("name",a,c.items)[0])return{category:c.name,item:a}}return null}function b(a,b){var c=document.getElementById(a);c.innerHTML="";for(var d=0;d<b.length;d++){var e=document.createElement("option");
...
"hidden";if((f=I.get_url_params())&&f.load){var g=f.load.split("/");sa={load_file:f.load,name:g[g.length-1].split(".")[0],animated_objects:[]}}else sa={load_file:M.get_std_assets_path()+"misc/logo.json",name:"Logo",animated_objects:[]};ba=G=null;if(!e)if(b){e=xa.keyfind("name",b.category,Y)[0];e=xa.keyfind("name",b.item,e.items)[0];for(var h in e)sa[h]="load_file"==h?M.get_std_assets_path()+e[h]:e[h];F.set("last_item_name",b.item)}else F.set("last_item_name","");c&&(ua.clear_render_callback(),Oa.cleanup());
wa.get_canvas().removeEventListener("mousedown",a);J.clear_errors_warnings();b=sa.name;c=sa.load_file;h=document.getElementById("current_file");h.innerHTML=b;h.setAttribute("title",b+" ("+c+")");Oa.load(c,l,p,d)}function l(b){wa.get_canvas().addEventListener("mousedown",a,!1);U=null;q(sa);g();I.enable_debug_controls();P()&&yb.enable_mixer_controls();"true"===F.get("gyro_use")&&zb.enable_camera_rotation();"HMD"==M.get("stereo")&&(Ia.update(),eb.enable_hmd(eb.HMD_ALL_AXES_MOUSE_NONE));ua.set_render_callback(w);
ya&&(Na=setTimeout(m,1E3));b=K.create_elapsed_sensor();K.create_sensor_manifold(A.get_active_camera(),"TO_OBJ",K.CT_CONTINUOUS,[b],null,function(a,b,d){if(1==d&&U){if(d=U)b=Ab,Bb.get_object_center(d,!0,b),a=cb.get_translation(a,Cb),db=Db.dist(b,a).toFixed(3);c()}})}function m(){var a=d();if(a){var b=a.item,a=xa.keyfind("name",a.category,Y)[0],b=xa.keyfind("name",b,a.items)[0],b=a.items.indexOf(b);if(b=a.items[b+1])a={category:a.name,item:b.name};else{a=Y.indexOf(a);b=null;do b=Y[++a];while(b&&0==
b.items.length);if(b)a={category:b.name,item:b.items[0].name};else{ya=!1;return}}}else a=Y[0],b=a.items[0],a={category:a.name,item:b.name};h(a,!0,!0,!1)}function p(a,b){document.getElementById("loading_progress").innerHTML=a+"% ("+Math.round(10*b/1E3)/10+"s)";var c=J.get_error_quantity(),d=J.get_warning_quantity();if(Oa.is_primary_loaded()){T();var e=J.num_vertices(),f=J.num_triangles(),g=J.num_draw_calls(),h=J.num_shaders();document.getElementById("info_right_up").innerHTML=e+" verts, "+f+" tris, "+
g+" draw calls, "+h+" shaders";var e=J.geometry_stats(),f=J.num_textures(),g=J.num_render_targets(),h=Math.round(10*(e.ibo_memory+e.vbo_memory))/10,k=Math.round(10*f.memory)/10,l=Math.round(10*g.memory)/10,m=Math.round(10*(h+k+l))/10;document.getElementById("info_right_down").innerHTML="Geometry: "+h+" MiB ("+e.ibo_number+"+"+e.vbo_number+")<br>Textures: "+k+" MiB ("+f.number+")<br>RTs: "+l+" MiB ("+g.number+")<br><br>Total: "+m+" MiB";e=J.get_error_quantity();(f=J.get_warning_quantity())||e?O.setAttribute("title",
...
a.dst_min_fac),u("waves_hor_fac",a.waves_hor_fac)):x(c,"disable")};Eb();a=A.get_all_objects("LAMP");c=[];for(e=0;e<a.length;e++)c.push(ka(a[e]));b("light_name",c);0<a.length&&gb(a[0]);Fb();Gb();Hb();Ib();Jb();Kb();Lb();Mb();Nb();Ob();Pb();Qb();bb();$("#debug_view_mode").val("DV_NONE");$("#debug_view_mode").selectmenu("refresh",!0);x(["debug_change_colors"],"disable");x(["render_time_threshold"],"disable");$("#wireframe_edge_color div").css("backgroundColor","#333");$("#render_time_threshold").trigger("change");
e=new Date(document.getElementById("date").value);Ka({date:e})}function fa(a,b){function c(a,b){for(var d in a)b[d]=a[d];return b}var d=b.scene_settings;if(d&&d[a]){var e={};c(b,e);c(d[a],e);delete e.scene_settings;return e}return b}function E(a,c){A.set_active(a);A.get_active_camera();for(var d=[],e=A.get_all_objects(),f=0;f<e.length;f++){var g=e[f];B.is_animated(g)&&(G||(G=g),d.push(ka(g)))}f="anim_active_object anim_slot animation anim_cyclic anim_frame_range anim_mix_factor anim_frame_current anim_play anim_stop".split(" ");
G?(x(f,"enable"),b("anim_active_object",d),na({anim_active_object:ka(G)})):x(f,"disable");d=["nla_play","nla_stop","nla_status","nla_frame_current","nla_playing"];f=["nla_frame_range"];if(S.check_nla()){S.check_nla_scripts()?(x(d,"disable"),document.getElementById("nla_status").innerHTML="CONTROLLED BY NLA SCRIPT"):x(d,"enable");x(f,"enable");var d=document.getElementById("nla_frame_current"),f=document.getElementById("nla_frame_range"),g=S.get_frame_start(),h=S.get_frame_end();-1!=g&&-1!=h?(f.innerHTML=
g+" - "+h,d.min=""+g,d.max=""+h):f.innerHTML="N/A"}else x(d,"disable"),x(f,"disable");d=[];for(f=0;f<e.length;f++)g=e[f],za.check_shape_keys(g)&&(ba||(ba=g),d.push(ka(g)));e=["shape_key_obj","shape_key_name","shape_key_value"];ba?(x(e,"enable"),b("shape_key_obj",d),Ba({shape_key_obj:ka(ba)})):x(e,"disable")}function w(a,b){A.get_active_camera();if(G){var c=document.getElementById("anim_status"),d=parseInt(document.getElementById("anim_slot").value);B.is_play(G,d)?(d=Math.round(B.get_frame(G,d)),parseInt($("#anim_frame_current").val())!==
d&&u("anim_frame_current",d),c.innerHTML="PLAYING"):c.innerHTML="STOPPED"}S.check_nla()&&(S.check_nla_scripts()||(c=document.getElementById("nla_status"),S.is_play()?c.innerHTML="PLAYING":c.innerHTML="STOPPED"),d=parseInt(S.get_frame().toFixed(0)),S.is_play()&&parseInt($("#nla_frame_current").val())!==d&&u("nla_frame_current",d))}function y(a){F.set("quality",a.quality);setTimeout(function(){window.location.reload()},100)}function z(){F.cleanup();var a=window.location.href.replace(/load=.*?(&|$)/,
"").replace(/(&|\?)$/,"");window.location.href=a}function C(){h(null,!0,!1,!0)}function v(){ua.pause()}function D(){ua.resume()}function P(){return"true"===F.get("sfx_mix_mode")?!0:!1}function Q(a){F.set("show_hud_debug_info",a.show_hud_debug_info);window.location.reload()}function L(a){F.set("enable_gl_debug",a.enable_gl_debug);window.location.reload()}function ha(a){F.set("sfx_mix_mode",a.sfx_mix_mode);window.location.reload()}function ia(a){F.set("stereo",a.stereo);window.location.reload()}function X(a){Ia.show("hmd_container")}
function Ga(a){F.set("gyro_use",a.gyro_use);window.location.reload()}function Z(a){a.gmpd_settings?hb.show():hb.hide()}function ma(a){F.set("outlining_overview_mode",a.outlining_overview_mode);window.location.reload()}function N(){ya=!0;m()}function T(a){wa.resize_to_container();a=wa.get_canvas();document.getElementById("info_left_up").innerHTML=a.width+"x"+a.height+" ("+(a.width/a.clientWidth).toFixed(2)+":1.00)"}function ca(){var a=document.getElementById("animation").value,b=parseInt(document.getElementById("anim_slot").value);
"None"!==a&&B.play(G,null,b)}function ga(){document.getElementById("animation");var a=parseInt(document.getElementById("anim_slot").value);B.stop(G,a)}function Fa(){for(var a=document.getElementById("anim_active_object").children,b=0;b<a.length;b++){var c=ta(a[b].value);B.play(c,null,B.SLOT_ALL)}}function lb(){for(var a=document.getElementById("anim_active_object").children,b=0;b<a.length;b++){var c=ta(a[b].value);B.stop(c,null,B.SLOT_ALL)}}function mb(){S.play()}function nb(){S.stop()}function Ba(a){if("shape_key_obj"in
a){ba=ta(a.shape_key_obj);var c=za.get_shape_keys_names(ba);c.length?(b("shape_key_name",c),Aa=c[0],c=za.get_shape_key_value(ba,Aa),u("shape_key_value",c)):b("shape_key_name",["N/K"])}"shape_key_name"in a&&(Aa=a.shape_key_name,c=za.get_shape_key_value(ba,Aa),u("shape_key_value",c));"shape_key_value"in a&&za.set_shape_key_value(ba,Aa,a.shape_key_value)}function ob(a){"nla_frame_current"in a&&!S.is_play()&&(a=parseInt(a.nla_frame_current),S.set_frame(a))}function na(a){if("anim_active_object"in a){G=
...
a&&(b.ssao_dist_factor=a.ssao_dist_factor);"ssao_only"in a&&(b.ssao_only=Number(a.ssao_only));"ssao_white"in a&&(b.ssao_white=Number(a.ssao_white));A.set_ssao_params(b)}function Jb(){var a=A.get_color_correction_params(),b=["brightness","contrast","exposure","saturation"];a?(x(b,"enable"),u("brightness",a.brightness),u("contrast",a.contrast),u("exposure",a.exposure),u("saturation",a.saturation)):x(b,"disable")}function Da(a){var b=A.get_color_correction_params();b&&(b={},"brightness"in a&&(b.brightness=
parseFloat(a.brightness)),"contrast"in a&&(b.contrast=parseFloat(a.contrast)),"exposure"in a&&(b.exposure=parseFloat(a.exposure)),"saturation"in a&&(b.saturation=parseFloat(a.saturation)),A.set_color_correction_params(b))}function ib(a){a=a.options;for(var b=0;b<a.length;b++){var c=a[b];if(c.selected)return c.value}}function Fb(){var a=A.get_shadow_params(),b="csm_first_cascade_border csm_last_cascade_border self_shadow_polygon_offset self_shadow_normal_offset first_cascade_blur_radius last_cascade_blur_radius res_ui-block-b num_ui-block-b blur_ui-block-b".split(" ");
...
"none",document.getElementById("first_cascade_blur_radius-label").innerHTML="Blur radius",document.getElementById("last_cascade_blur_radius-info-block").style.display="none"}function Eb(){var a=A.get_environment_colors();u("environment_energy",a[0]);la("horizon_color",a[1]);la("zenith_color",a[2])}function Ja(a){("environment_energy"in a||"horizon_color"in a||"zenith_color"in a)&&A.set_environment_colors(a.environment_energy,a.horizon_color,a.zenith_color)}function $a(a){var b={};if("string"==typeof a.debug_view_mode){var c=
...
J.DV_BATCHES_VIEW;break;case "DV_RENDER_TIME":b.debug_view_mode=J.DV_RENDER_TIME}"DV_CLUSTERS_VIEW"==c||"DV_BATCHES_VIEW"==c?x(["debug_change_colors"],"enable"):x(["debug_change_colors"],"disable");"DV_RENDER_TIME"==c?x(["render_time_threshold"],"enable"):x(["render_time_threshold"],"disable")}"object"==typeof a.wireframe_edge_color&&(b.wireframe_edge_color=a.wireframe_edge_color);J.set_debug_params(b)}function ub(){J.set_debug_params({debug_colors_seed:Math.random()})}function vb(a){"render_time_threshold"in
a&&J.set_debug_params({render_time_threshold:a.render_time_threshold})}function wb(){Rb.shot()}function Mb(){var a=A.get_sky_params(),b="sky_color rayleigh_brightness mie_brightness spot_brightness scatter_strength rayleigh_strength mie_strength rayleigh_collection_power mie_collection_power mie_distribution".split(" ");if(!a)return x(b,"disable"),null;x(b,"enable");la("sky_color",a.color);u("rayleigh_brightness",a.rayleigh_brightness);u("mie_brightness",a.mie_brightness);u("spot_brightness",a.spot_brightness);
u("scatter_strength",a.scatter_strength);u("rayleigh_strength",a.rayleigh_strength);u("mie_strength",a.mie_strength);u("rayleigh_collection_power",a.rayleigh_collection_power);u("mie_collection_power",a.mie_collection_power);u("mie_distribution",a.mie_distribution)}function aa(a){var b={};"sky_color"in a&&(b.color=a.sky_color);"rayleigh_brightness"in a&&(b.rayleigh_brightness=a.rayleigh_brightness);"mie_brightness"in a&&(b.mie_brightness=a.mie_brightness);"spot_brightness"in a&&(b.spot_brightness=
a.spot_brightness);"scatter_strength"in a&&(b.scatter_strength=a.scatter_strength);"rayleigh_strength"in a&&(b.rayleigh_strength=a.rayleigh_strength);"mie_strength"in a&&(b.mie_strength=a.mie_strength);"rayleigh_collection_power"in a&&(b.rayleigh_collection_power=a.rayleigh_collection_power);"mie_collection_power"in a&&(b.mie_collection_power=a.mie_collection_power);"mie_distribution"in a&&(b.mie_distribution=a.mie_distribution);A.set_sky_params(b)}function fb(a,b){var c=qa.get_material_extended_params(a,
b),d="material_diffuse_color material_name material_reflectivity material_fresnel material_fresnel_factor material_parallax_scale material_parallax_steps".split(" ");if(!c)return x(d,"disable"),$("#material_warning")[0].style.display="block",null;var e=qa.get_diffuse_color(a,b);c.diffuse_color=e.subarray(0,3);x(d,"enable");la("material_diffuse_color",c.diffuse_color);u("material_reflectivity",c.reflect_factor);u("material_fresnel",c.fresnel);u("material_fresnel_factor",c.fresnel_factor);u("material_parallax_scale",
...
a&&(c.material_parallax_scale=a.material_parallax_scale);"material_parallax_steps"in a&&(c.material_parallax_steps=a.material_parallax_steps);qa.set_material_extended_params(b,$("#material_name").val(),c);A.update_scene_materials_params()}function H(a){var b=U,c="waves_height waves_length sss_strength sss_width dst_noise_scale0 dst_noise_scale1 dst_noise_freq0 dst_noise_freq1 dir_min_shore_fac dir_freq dir_noise_scale dir_noise_freq dir_min_noise_fac dst_min_fac waves_hor_fac".split(" ");if(!b)return null;
...
null}a(e)})}function da(a,b){$("#"+b).ColorPicker({onChange:function(c,d,e){$("#"+b+" div").css("backgroundColor","#"+d);c={};e=Ra.css_to_rgb(e.r,e.g,e.b);c[b]=[e[0],e[1],e[2]];a(c)}})}function pb(){Ub.auto_rotate(.3)}function x(a,b){if(!a)return null;for(var c=0;c<a.length;c++){var d=$("#"+a[c]).parent().parent();"enable"==b?d.removeClass("ui-disabled"):"disable"==b&&d.addClass("ui-disabled")}}function Ta(a,b){if(!a)return null;for(var c=0;c<a.length;c++){var d=$("#"+a[c]);"enable"==b?d.removeClass("ui-disabled"):
"disable"==b&&d.addClass("ui-disabled")}}function ka(a){return A.get_object_name_hierarchy(a).join("->")}function ta(a){return A.get_object_by_dupli_name_list(a.split("->"))}var B=n("animation"),I=n("app"),Ua=n("assets"),cb=n("camera"),Ub=n("camera_anim"),M=n("config");n("constraints");var wa=n("container"),K=n("controls"),Oa=n("data"),J=n("debug"),hb=n("gp_conf"),Ia=n("hmd_conf"),za=n("geometry"),zb=n("gyroscope"),Xa=n("input"),V=n("lights"),ua=n("main"),eb=n("hmd"),yb=n("mixer"),qa=n("material"),
S=n("nla"),xb=n("objects"),Ra=n("rgb"),Rb=n("screenshooter"),A=n("scenes"),F=n("storage"),Bb=n("transform"),xa=n("util"),Va=n("version"),Db=n("vec3"),kb="DEBUG"===Va.type(),Tb=Math.PI/180,Sb=180/Math.PI,Ab=new Float32Array(3),Cb=new Float32Array(3),ya=!1,Na,Y=null,G=null,ba=null,Aa="",sa=null,Ca=null,O=null,U=null,ab=function(){},ja=null,db=null;k.init=function(){F.init("b4w_viewer");var a=F.get("quality");if(a){switch(a){case "ULTRA":var b=M.P_ULTRA;break;case "HIGH":b=M.P_HIGH;break;case "LOW":b=
M.P_LOW;break;case "CUSTOM":b=M.P_CUSTOM}M.set("quality",b)}M.set("stereo",F.get("stereo")||"NONE");""==F.get("outlining_overview_mode")?M.set("outlining_overview_mode",!0):M.set("outlining_overview_mode","true"===F.get("outlining_overview_mode"));I.init({canvas_container_id:"main_canvas_container",callback:t,autoresize:!1,gl_debug:""==F.get("enable_gl_debug")?!0:"true"===F.get("enable_gl_debug"),show_hud_debug_info:"true"===F.get("show_hud_debug_info")?!0:!1,sfx_mix_mode:P(),show_fps:!0,fps_elem_id:"fps_logger",
