...
function enable_default_rotation(elem, character) {
    if (check_pointerlock(elem) && !m_main.detect_mobile()) {
        ...
        var fps_plock_sensors_cb = function(obj, id, pulse) {
            if (pulse > 0) {
                ...
                default_rotation_cb(obj, -payload.coords[0] * _fps_camera_mult,
        ...
        m_ctl.create_sensor_manifold(character, "FPS_SMOOTH_DRAG", m_ctl.CT_CONTINUOUS,
            [elapsed], null, smooth_cb, default_rotation_cb);
    }
}

function set_characters_camera(character) {
...
function enable_camera_rotation(elem, character) {
    if (check_vr_support()) {
        ...
        enable_default_rotation(elem, character);
    }
}
...
function enable_movements(elem, character, motion_cb) {
    ...
    var change_movestyle = function(value) {
        if (move_state.forw + move_state.back +
                move_state.left + move_state.right) {
            if (value > 0.0)
    ...
    var forward_sensors_array = [m_ctl.KEY_W, m_input.GMPD_BUTTON_12];
    var backward_sensors_array = [m_ctl.KEY_S, m_input.GMPD_BUTTON_13];
    if (m_main.detect_mobile()) {
        forward_sensors_array.push(MOBILE_FORWARD_BTN_ID);
        backward_sensors_array.push(MOBILE_BACKWARD_BTN_ID);
    }
    bind_action(AT_PRESSED, forward_sensors_array, move_forward_cb);
...
function init_character_states(character) {
    ...
    _character_sm = create_state_machine();
    ...
    state_machine_add_state(sm, CS_STAY, [CS_WALK, CS_FLY, CS_CLIMB], change_state_cb, 
            null, null);
...
/**
 * Enable FPS controls. This sets default keyboard, gamepad and mouse controls.
...
exports.enable_fps_controls = function(character, elem, motion_cb) {
    var character = character || m_scs.get_first_character();
    var elem = elem || m_cont.get_canvas();

    if (!character)
        return;

    var motion_cb = motion_cb || null;

    set_characters_camera(character);
...
exports.disable_fps_controls = function(character, elem) {
    var character = character || m_scs.get_first_character();
    var elem = elem || m_cont.get_container();
