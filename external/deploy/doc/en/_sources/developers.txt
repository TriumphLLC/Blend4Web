...
.. code-block:: bash
    ...
    ./project.py -a my_app_path -o advanced -b copy
...
* Опция ``"-o"`` (необязательная) используется для указания метода оптимизации js-файлов.
  Доступные варианты: **simple** (по умолчанию) и **advanced**.
* Опция ``"-b"`` (необязательная) указывает из какой директории брать движок.
  ...
  ``external/deploy/apps/common/``, **copy** - движок копируется в директорию
  с собранным приложением, **combine** - движок объединяется с минимизированным
  скриптом в корне приложения.
...
* Приложение полностью копируется в директорию ``external/deploy/apps/app_name/``.
...
Конвертация медиа ресурсов происходит с потерей качества, поэтому сконвертированные
аудио и видео файлы получают суффикс **.lossconv**.
