...
GET_EXPORT_HTML_PATH = "blender_scripts/addons/blend4web/command_line_get_export_html_path.py"
...
def reexport_html(path):
    ...
    blend = os.path.abspath(os.path.normpath(os.path.join(os.path.dirname(path), blend_path)))
    get_export_html_path_script = os.path.abspath(GET_EXPORT_HTML_PATH)
    reexporter_script = os.path.abspath(HTML_REEXPORTER)

    if not os.path.isfile(blend):
        print (RED + " [NO BLEND]" + ENDCOL, path)
        return

    ret = subprocess.check_output(["blender", "-b", blend, "-P", get_export_html_path_script], stderr=subprocess.STDOUT)
